    ...
    public void update() {
        if (this.duration == this.startDuration) {
            if (CompanionField.currCompanion.get(AbstractDungeon.player) != null) {
                if (CompanionField.currCompanion.get(AbstractDungeon.player) instanceof Outcast) {
                    this.isDone = true;
                    return;
                }
            ...
            if (AbstractDungeon.getCurrRoom() instanceof MonsterRoom && lastCombatMetricKey.equals("Shield and Spear")) {
                this.c = new Outcast(AbstractCompanion.INIT_X - Settings.WIDTH * 0.25F, AbstractCompanion.INIT_Y);
            } else {
                this.c = new Outcast(AbstractCompanion.INIT_X - Settings.WIDTH * 0.5F, AbstractCompanion.INIT_Y);
            }
